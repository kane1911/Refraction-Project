...
def main():
    ...
    try:
        if type(incidence_angle) == str and incidence_angle.isalpha(): # if the user enters only letters
            tkinter.messagebox.showerror(title="Letter(s) entered",
                                             message="Please enter an incidence angle between -90° and 90°.")  # displays error pop-up window
            ...
            calculated_angle = tkinter.Label(frame, text=str(r_angle) + "°")  # concatenates a degree symbol
            calculated_angle.grid(row=5, column=2)
